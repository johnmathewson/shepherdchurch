...
export default function PrayerWeekPage() {
  ...
  const coveragePct = slots.length > 0 ? Math.round((filledCount / slots.length) * 100) : 0
  ...
  return (
    ...
    <div className="min-h-screen page-bg">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
        ...
        {/* Coverage bar — every one of the 168 hours is a cell */}
        <div className="glass-elevated max-w-3xl mx-auto p-5 sm:p-6 mb-10 animate-fade-in">
          <div className="flex items-end justify-between mb-4 gap-4">
            <div>
              <div className="font-heading flex items-baseline gap-1">
                <span className="text-gold text-3xl sm:text-4xl font-bold tabular-nums">{filledCount}</span>
                <span className="text-text-muted text-xl sm:text-2xl font-medium tabular-nums">/{slots.length}</span>
              </div>
              <div className="text-text-muted text-[10px] uppercase tracking-[0.18em] mt-1">
                Hours of unbroken prayer
              </div>
            </div>
            <div className="text-right">
              <div className="font-heading text-3xl sm:text-4xl font-bold text-text-primary tabular-nums">{coveragePct}%</div>
              <div className="text-text-muted text-[10px] uppercase tracking-[0.18em] mt-1">Covered</div>
            </div>
          </div>

          {/* The 168-cell strip itself */}
          <div className="flex h-9 sm:h-10 rounded-md overflow-hidden border border-white/10 shadow-inner bg-white/[0.02]">
            {slots.map((slot) => {
              const filled = slot.public_status === 'filled'
              ...
              return (
                <div
                  ...
                  title={`${formatFullTime(new Date(slot.starts_at))} — ${mine ? 'claimed by you' : filled ? 'covered' : 'open'}`}
                />
              )
            })}
          </div>

          {/* Day labels — 7 evenly spaced columns align with each 24-hour block */}
          <div className="grid grid-cols-7 mt-2 text-[10px] text-text-muted text-center uppercase tracking-[0.18em]">
            {days.map(d => (
              <div key={d.date.toISOString()}>{formatDay(d.date)}</div>
            ))}
          </div>

          {/* Legend + personal callout */}
          <div className="flex items-center justify-center gap-4 sm:gap-6 mt-5 text-[11px] text-text-muted flex-wrap">
            <span className="inline-flex items-center gap-1.5">
              <span className="w-3 h-3 rounded-sm bg-sage"></span>Covered
            </span>
            <span className="inline-flex items-center gap-1.5">
              <span className="w-3 h-3 rounded-sm bg-white/10 border border-white/15"></span>Open
            </span>
            {authed && myCount > 0 && (
              <span className="inline-flex items-center gap-1.5 text-gold">
                <span className="w-3 h-3 rounded-sm bg-gold"></span>
                <span className="font-semibold">{myCount}</span> claimed by you
              </span>
            )}
          </div>
        </div>
